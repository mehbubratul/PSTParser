...
public class DSV {
    ...
    private ArrayList<Object> fields; //String to ?
    private ArrayList<Object> headerFields;
            ...
            throws java.io.IOException
    {
        ...
        if ( (b[0] & 0xFF) == 0xEF && len == 3 ) {
            ...
            if ( (b[0] & 0xFF) == 0xFE &&
                    (b[1] & 0xFF) == 0xFF ) {
                return new InputStreamReader(pin, StandardCharsets.UTF_16BE);
            } else if ( (b[0] & 0xFF) == 0xFF &&
                    (b[1] & 0xFF) == 0xFE ) {
                return new InputStreamReader(pin, StandardCharsets.UTF_16LE);
            } else {
                pin.unread(b, 0, len);
            }
        } else if ( len > 0 ) {
    ...
    public DSV(boolean stripMultipleNewlines,
               char separator,
               InputStream input)
            throws java.io.IOException
    {
            ...
            throws java.io.IOException
    {
        ...
        this.headerFields = new ArrayList<>();
        this.eofSeen = false;
        this.in = new BufferedReader(stripBom(bis));
    }
    ...
    public boolean hasHeaders() throws java.io.IOException
    {
        if ( eofSeen ) return false;
        headerFields.clear();
        eofSeen = split( in, headerFields );
        if ( eofSeen ) return ! headerFields.isEmpty();
        else return true;
    }
    ...
    private boolean split(Reader in,ArrayList<Object> fields)
            throws java.io.IOException
    {
